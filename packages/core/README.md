...
- **Uniform borders only** -- reads the top border and applies it uniformly
- **Solid borders only** -- `dashed`, `dotted`, `double`, etc. are not replicated
- **First shadow only** -- multiple `box-shadow` layers: only the first outer and first inset are extracted
