...
- ee659b8: **`generatePath` is 4.5× faster on cached calls and 2.6× faster cold.** Same bytes out — all 8415 cases of a golden corpus (every curve, radius, smoothing, exponent, per-corner mix, the capsule and blend bands swept at 0.5 px, and non-finite inputs) are byte-identical, digest for digest.
